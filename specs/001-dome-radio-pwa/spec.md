...
- **NFR-SEC-001**: On HTTPS origins, station artwork and other remote images rendered by the app MUST NOT be requested over plain `http://` (upgrade to `https://` or omit / fall back) so the page does not trigger mixed-content warnings for images. Live stream URLs MUST prefer `https://` first; when the catalog only provides `http://` (or protocol-relative) and HTTPS playback fails, the player MAY fall back to the insecure stream. HTTP-only fallback MAY show a browser mixed-content indicator; an HTTPS stream proxy is out of scope for v1.
